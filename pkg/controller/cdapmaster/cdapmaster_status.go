...
import (
	"cdap.io/cdap-operator/pkg/apis/cdap/v1alpha1"
	"time"
...
func (s *Base) UpdateStatus(rsrci interface{}, reconciled []reconciler.Object, err error) time.Duration {
	...
	master := rsrci.(*v1alpha1.CDAPMaster)
	stts := &master.Status
...
func updateStatus(rsrci interface{}, reconciled []reconciler.Object, err error) time.Duration {
	var period time.Duration
	master := rsrci.(*v1alpha1.CDAPMaster)
	stts := &master.Status
	ready := stts.ComponentMeta.UpdateStatus(reconciler.ObjectsByType(reconciled, k8s.Type))
